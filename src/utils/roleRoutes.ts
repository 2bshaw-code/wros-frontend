...
export const isFounder = (user: ConsoleUser | null) => Boolean(
  user?.founder || user?.role === 'founder' || user?.role === 'founder_admin' || user?.role === 'founder_master' || user?.operatorRole === 'founder' || user?.operatorRole === 'founder_admin' || user?.operatorRole === 'founder_master'
)
